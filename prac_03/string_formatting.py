"""
  ...
  print "my guitar: " + name + ", first made in " + to_string(year)
  print "my " + name + " was first made in " + to_string(year) + " (that's right, " + to_string(year) + "!)"
  print name + " would cost $" + format(cost, ",.2f")

  numbers ← [1, 19, 123, 456, -25]
  for i, n in enumerate(numbers, 1)
    print "number " + to_string(i) + " is " + pad_left(to_string(n), 5)

  print to_string(year) + " " + name + " for about $" + format(cost, ",.0f") + "!"

  for e from 0 to 9
    v ← 2 ^ e
    print "2 ^ " + pad_left(to_string(e),2) + " is " + pad_left(to_string(v),4)
"""
